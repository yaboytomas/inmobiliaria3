...
ALTER USER PRUEBA3 QUOTA UNLIMITED ON SYSAUX;
GRANT CONNECT TO PRUEBA3  ;
GRANT RESOURCE TO PRUEBA3 ;
...
SELECT 
    cli.numrut_cli || '-' || cli.dvrut_cli AS "RUT CLIENTE",
    cli.appaterno_cli || ' ' || cli.apmaterno_cli || ' ' || cli.nombre_cli AS "NOMBRE CLIENTE",
    cli.fonofijo_cli AS "TELEFONO FIJO",
    cli.celular_cli CELULAR,
    cli.renta_cli as "RENTA ESPERADA",
    ec.desc_estcivil as "ESTADO CIVIL"
FROM cliente cli
INNER JOIN estado_civil ec on cli.id_estcivil = ec.id_estcivil
WHERE (ec.desc_estcivil = 'Soltero')
   OR (ec.desc_estcivil IN ('Divorciado', 'Separado') AND cli.renta_cli >= 800000)
ORDER BY cli.appaterno_cli, cli.nombre_cli;
